...
> Agentic CLI Usage:
> 1. [`AGENTS.md`](AGENTS.md) as the agent hub for using the `docwarp` CLI.
> 2. `README.md` for commands and conversion flow.
> 3. [`docs/strict-mode.md`](docs/strict-mode.md) for CI/quality gates.

## Features
- **Built for Agents:** CLI with instructions for agentic usage.
- **Bidirectional:** `md ⇄ docx` round-trip reliability.
- **Math/Equations:** native Word equations for math and statistical expressions.
- **Template Support:** select your company Word template to preserve styles.
- **Password Protected Docs:** convert password protected DOCX files with password input.
...
## Installation - Current Version v0.1.2
...
### Windows (.exe)
```bash
https://github.com/N10ELabs/docwarp/releases
```
...
## Create Backups to Prevent Overwriting
